...
class ComprehensiveStockAnalyzer:
    ...
    def run_full_analysis(self, min_market_cap=300e6, max_tickers=1000, cap_allocation='balanced'):
...
class QuarterlyRebalancer:
    ...
    def get_sector_etf(self, sector):
    ...
    def calculate_spread_zscore(self, stock_ticker, etf_ticker, period='2y'):
        ...
        try:
            ...
            _, pvalue, _ = coint(log_stock, log_etf)
            is_cointegrated = pvalue < 0.05
            
            recent = aligned.tail(self.lookback)
            beta = np.cov(recent['stock'], recent['etf'])[0,1] / np.var(recent['etf'])
